...
Для переноса старого Octopus у аккаунта есть необязательный уникальный `old_id`. Команда `import-octopus-users` читает старую БД, проверяет email, статусы и bcrypt-хеши, создаёт UUIDv7 и назначения `main_api`, а по умолчанию работает как dry-run. Повторный запуск идемпотентен. Унаследованный bcrypt-хеш после первого успешного входа заменяется на Argon2id. Новые аккаунты создаются без `old_id`.
